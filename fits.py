...
def line_fit_Data(*Dat):
    ...
    # Chiral extrap of Z^{-1}. (g,g)
    points = [(d.m + mres, d.Zinv, d.Zinv_sigmaJK) for d in Dat]
    result.Zinv, result.Zinv_sigmaJK = line_fit(points)
    
    # Chiral extrap of Z^{-1}. (q,q)    
    points = [(d.m + mres, d.Zinv_q, d.Zinv_q_sigmaJK) for d in Dat]
    result.Zinv_q, result.Zinv_q_sigmaJK = line_fit(points)
...
def line_fit_bilinears(*Dat):
    ...
    # Chiral extrap of Z_P.
    result.Z_P = dict(g=None, q=None)
    result.Z_P_sigmaJK = dict(g=None, q=None)
    result.Z_P_fit = dict(g=None, q=None)
    for scheme in 'g', 'q':
        points = [(d.m + mres, d.Z_P[scheme], d.Z_P_sigmaJK[scheme]) for d in Dat]
        fit = line_fit2(points)
        result.Z_P[scheme], result.Z_P_sigmaJK[scheme] = fit.a, fit.sig_a
        result.Z_P_fit = fit
